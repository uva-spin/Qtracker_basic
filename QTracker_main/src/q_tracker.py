# TODO: Need to update based on multi-track finder changes
...
from src.preprocessing.data_loader import load_detector_element_data
...
def process_data(root_file, output_file="tracker_output.root", use_chi2_model=USE_CHI2):
    ...
    rHitArray_mup, rHitArray_mum, softmax_mup, softmax_mum = predict_hit_arrays(
        model_track, X
    )

    refiner = Refiner()
    refined_HitArray_mup, refined_HitArray_mum = refiner.refine_hit_arrays(
    ...
    results = {
        "momentum_mup": predict_momentum(
            refined_HitArray_mup_with_drift, model_momentum_mup
        ),
        "momentum_mum": predict_momentum(
            refined_HitArray_mum_with_drift, model_momentum_mum
        ),
    }

    # chi^2 computation (optional)
    if use_chi2_model:
        combined_hit_arrays = np.concatenate(
            [refined_HitArray_mup, refined_HitArray_mum], axis=0
        )
        combined_momentum_vectors = np.concatenate(
            [results["momentum_mup"], results["momentum_mum"]], axis=0
        )
        chi2_predictions = predict_chi2(
            combined_hit_arrays,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run track finder and momentum inference on ROOT file."
    )
    parser.add_argument("root_file", type=str, help="Path to input ROOT file.")
    parser.add_argument(
        "--output_file",
        type=str,
        default="qtracker_reco.root",
