...
from src.config import SINGLE_TRACK_FINDER_PATH, MAX_STEPS
...
class MultiTrackFinder:
    def __init__(
        ...
        Args:
            max_steps (int): Maximum number of steps for the iterative track finding process. Should match data generation.
            mode (Literal["evaluation", "production"]): Mode of operation, either "evaluation" or "production".
        """
    ...
    def run(
        ...
        for _ in range(self.max_steps):
            # Early exit allowed only for deployment
            ...
            # Update per event activity
            active_events &= confidence_score >= 0.5

            mp_softmax = y_pred[:, 0, :, :]
            mm_softmax = y_pred[:, 1, :, :]

            mu_plus_pred = tf.argmax(mp_softmax, axis=-1).numpy().astype(np.int32)
            mu_minus_pred = tf.argmax(mm_softmax, axis=-1).numpy().astype(np.int32)
            ...
            mp_softmax[~active_events] = 0.0
            mm_softmax[~active_events] = 0.0
            ...
            # Subtract only for active events
    ...
    def evaluate(self, input_root_file: str) -> dict[str, Union[float, list[float]]]:
        ...
        for evt in range(mu_plus_residuals.shape[0]):
            evt_mask = mask[evt]  # shape (max_steps, 62)
    ...
    def _subtract_found_tracks(
        ...
        for event_idx in range(num_events):
            # Skip inactive events entirely
            if not active_events[event_idx]:
                continue
