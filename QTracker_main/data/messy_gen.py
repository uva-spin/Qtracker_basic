...
def inject_tracks(
    ...
    if total_len_mup % NUM_LAYERS != 0 or total_len_mum % NUM_LAYERS != 0:
        raise RuntimeError(
            "HitArray_mup/mum leaf length is not a multiple of NUM_LAYERS"
        )
    ...
    output_tree.Branch(
        "HitArray_mup", HitArray_mup, f"HitArray_mup[{max_mup_tracks}][{NUM_LAYERS}]/I"
    )
    output_tree.Branch(
        "HitArray_mum", HitArray_mum, f"HitArray_mum[{max_mum_tracks}][{NUM_LAYERS}]/I"
    )
