...
This repository provides a full pipeline for preparing signal and background datasets for training in dimuon track reconstruction tasks for QTracker.py.
...
- `num_tracks 50` — specify the upper bound number of background tracks to inject (default: 50)
- `lower_bound 0` — specify the lower bound number of background tracks to inject (default: 0)
- `uniform_tracks 1` — flag to use uniform distribution for track injection (1 = True (default), 0 = False)
