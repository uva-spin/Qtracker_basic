...
def split_tracks(input_filename):
    ...
    output_file1 = ROOT.TFile(
        input_filename.replace(".root", "_track1.root"), "RECREATE"
    )
    output_file2 = ROOT.TFile(
        input_filename.replace(".root", "_track2.root"), "RECREATE"
    )
    ...
    for i in range(n_entries):
        ...
        # Clear vectors
        for v in vecs1.values():
            v.clear()
        for v in vecs2.values():
            v.clear()
