...
def momentum_residuals(root_file: str, mom_res_stats: str) -> None:
    ...
    gpx_muMinus, gpy_muMinus, gpz_muMinus = (
        np.array(gpx_muMinus),
        np.array(gpy_muMinus),
        np.array(gpz_muMinus),
    )
    qpx_muMinus, qpy_muMinus, qpz_muMinus = (
        np.array(qpx_muMinus),
        np.array(qpy_muMinus),
        np.array(qpz_muMinus),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="results/mom_res_stats.txt",
        help="Output file name for storing residual stats",
    )
