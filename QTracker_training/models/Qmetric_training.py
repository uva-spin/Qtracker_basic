...
def load_root_file(root_file):
    ...
    for event in tree:
        ...
        # Hit arrays
        hit_array_mup = np.array(
            event.qHitArray_mup, dtype=np.float32
        )  # Hit array for mu+
        hit_array_mum = np.array(
            event.qHitArray_mum, dtype=np.float32
        )  # Hit array for mu-
...
def train_chi2_model(chi2_values, hit_arrays, momentum_vectors, occupancies=None):
    ...
    optimizer = tf.keras.optimizers.AdamW(
        learning_rate=0.0003, weight_decay=1e-4, clipnorm=1.0
    )
    model.compile(
        optimizer=optimizer, loss=tf.keras.losses.Huber(delta=10.0), metrics=["mae"]
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Calculate chi^2 and train a model to predict it."
    )
