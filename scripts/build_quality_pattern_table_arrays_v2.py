...
def score_v2(row):
    """
    Local-density QMetric candidate score.

    This keeps the interpretable v0 structure but replaces the global
    event-occupancy penalty with a local-density penalty around the
    candidate hits. In the single-track validation ablation, this kept
    comparable hit quality while improving downstream momentum error.

    This is still a development/calibration score, not a final production
    guarantee. Truth-based columns must never be used in this score.
    """
    missing = float(row.get("missing_fraction_active", 0.0))
    station = float(row.get("station_coverage_fraction", 0.0))
    gap = min(float(row.get("max_missing_gap_active", 0.0)) / 20.0, 1.0)
    local_density = min(float(row.get("local_density_mean", 0.0)) / 5.0, 1.0)
    conf = float(row.get("softmax_conf_mean", 0.0))
    margin = float(row.get("softmax_margin_mean", 0.0))
    entropy = float(row.get("softmax_entropy_mean", 0.0))

    score = 1.0
    score -= 0.55 * missing
    score -= 0.20 * (1.0 - station)
    score -= 0.10 * gap
    score -= 0.06 * local_density
    score += 0.15 * conf
    score += 0.15 * margin
    score -= 0.15 * entropy
    return max(0.0, min(1.0, score))
...
def make_candidate_row(event_id, pair_index, charge_name, hit_array, softmax, event_matrix, truth_array, local_radius):
    ...
    row["qmetric_score_v2"] = score_v2(row)
    return row
