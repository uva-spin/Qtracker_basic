...
def _read_scalar(tree, branch_name, event_number, default_value):
    ...
    arr = tree[branch_name].array(entry_start = event_number, entry_stop = event_number + 1, library = "np")
    if arr.size == 0:
        return default_value
    return int(arr[0])
...
def plot_event(file_name, event_number, num_detectors = 62, num_elements = 201):
    with uproot.open(file_name) as file:
        ...
        for i in range(n_mup_tracks):
            legend_handles.append(mpatches.Patch(color = colors[1 + i], label = f"mu+ #{i + 1}"))
        for i in range(n_mum_tracks):
            legend_handles.append(mpatches.Patch(color = colors[1 + n_mup_tracks + i], label = f"mu- #{i + 1}"))
        legend_handles.append(mpatches.Patch(color = "black", label = "overlap (any tracks)"))

        if len(legend_handles) <= 20:
            plt.legend(handles = legend_handles, loc = "upper right")
        else:
            plt.legend(handles = [mpatches.Patch(color = "black", label = "overlap (any tracks)")],
                       loc = "upper right")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description = "Plot GT mu+/mu- hits per track (pixel), overlap -> black.")
    ...
    parser.add_argument("--num_detectors", type = int, default = 62, help = "Number of detectors (default: 62).")
    parser.add_argument("--num_elements", type = int, default = 201, help = "Number of element IDs (default: 201).")
    args = parser.parse_args()

    plot_event(args.file, args.event, num_detectors = args.num_detectors, num_elements = args.num_elements)
