...
def inject_tracks(file1, file2, output_file, num_tracks, lower_bound = 0, uniform_tracks = True):
    ...
    leaf_mup = tree1.GetLeaf("HitArray_mup")
    leaf_mum = tree1.GetLeaf("HitArray_mum")
    if not leaf_mup or not leaf_mum:
        raise RuntimeError("HitArray_mup or HitArray_mum leaf not found in input tree")

    total_len_mup = leaf_mup.GetLen()
    total_len_mum = leaf_mum.GetLen()

    if total_len_mup % NUM_LAYERS != 0 or total_len_mum % NUM_LAYERS != 0:
        raise RuntimeError("HitArray_mup/mum leaf length is not a multiple of NUM_LAYERS")

    max_mup_tracks = total_len_mup // NUM_LAYERS
    max_mum_tracks = total_len_mum // NUM_LAYERS
    ...
    eventID = array('i', [0])
    nMupTracks = array('i', [0])
    nMumTracks = array('i', [0])
    ...
    HitArray_mup = np.zeros((max_mup_tracks, NUM_LAYERS), dtype = np.int32)
    HitArray_mum = np.zeros((max_mum_tracks, NUM_LAYERS), dtype = np.int32)

    # Output tree branches
    output_tree.Branch("eventID", eventID, "eventID/I")
    output_tree.Branch("nMupTracks", nMupTracks, "nMupTracks/I")
    output_tree.Branch("nMumTracks", nMumTracks, "nMumTracks/I")
    ...
    output_tree.Branch("HitArray_mup", HitArray_mup, f"HitArray_mup[{max_mup_tracks}][{NUM_LAYERS}]/I")
    output_tree.Branch("HitArray_mum", HitArray_mum, f"HitArray_mum[{max_mum_tracks}][{NUM_LAYERS}]/I")

    # Bind input HitArray branches to NumPy buffers
    HitArray_mup_input = np.zeros((max_mup_tracks, NUM_LAYERS), dtype = np.int32)
    HitArray_mum_input = np.zeros((max_mum_tracks, NUM_LAYERS), dtype = np.int32)
    ...
    for i in range(tree1.GetEntries()):
        ...
        nMupTracks[0] = int(tree1.nMupTracks)
        nMumTracks[0] = int(tree1.nMumTracks)
        muID.clear()
        elementID.clear()
        detectorID.clear()
        ...
        for v in tree1.muID:
            muID.push_back(v)
        for v in tree1.gCharge:
            gCharge.push_back(v)
        for v in tree1.gTrackID:
            gTrackID.push_back(v)
        ...
        for k in range(max_mup_tracks):
            for l in range(NUM_LAYERS):
                HitArray_mup[k, l] = HitArray_mup_input[k, l]
        for k in range(max_mum_tracks):
            for l in range(NUM_LAYERS):
                HitArray_mum[k, l] = HitArray_mum_input[k, l]
