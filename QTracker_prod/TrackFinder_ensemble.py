...
def plot_residuals(det_ids, res_plus, res_minus, stage_label):
    ...
    fname    = f"TrackFinder_ensemble_{stage_label}_residuals.png"
...
def ensemble_model(args):
    X_test, X_occupancies, y_muPlus_test, y_muMinus_test = (
        data_loader.load_data_ensemble(args.root_file)
    )
    if X_test is None:
        return
    ...
    with tf.keras.utils.custom_object_scope(custom_objects):
        model1 = tf.keras.models.load_model(args.model1_path)
        model2 = tf.keras.models.load_model(args.model2_path)
        model3 = tf.keras.models.load_model(args.model3_path)

    y_pred = []
    for x, x_occ in zip(X_test, X_occupancies):
        x1 = x[None, ...]
        if x_occ < 330:
            y_pred.append(model1.predict(x1, verbose=0)[0])
        elif x_occ < 600:
            y_pred.append(model2.predict(x1, verbose=0)[0])
        else:
            y_pred.append(model3.predict(x1, verbose=0)[0])
    ...
    plot_residuals(dets_used, raw_p_res[:,mask], raw_m_res[:,mask], 'raw')
    ...
    dets_used = (np.where(mask)[0] + 1)
    plot_residuals(dets_used, ref_p_res[:,mask], ref_m_res[:,mask], 'refined')
...
if __name__ == '__main__':
    ...
    parser.add_argument("model1_path", type=str, help="Path to the clean saved model file (.h5 or .keras).")
    parser.add_argument("model2_path", type=str, help="Path to the medium saved model file (.h5 or .keras).")
    parser.add_argument("model3_path", type=str, help="Path to the noisy saved model file (.h5 or .keras).")
    args = parser.parse_args()

    print(f"\nResults for TrackFinder_ensemble...")
    ensemble_model(args)
