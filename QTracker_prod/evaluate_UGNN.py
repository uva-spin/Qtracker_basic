import os
import sys
import argparse
import absl.logging
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'
absl.logging.set_verbosity('error')

import ROOT  
import numpy as np
import tensorflow as tf
import matplotlib.pyplot as plt
import pandas as pd

ROOT_DIR = os.path.dirname(os.path.abspath(__file__))
if ROOT_DIR not in sys.path:
    sys.path.insert(0, ROOT_DIR)

from models import data_loader
from models import TrackFinder_unetpp
from models.losses import custom_loss
import QTracker_prod
from refine import refine_hit_arrays

from models.TrackFinder_UGNN_v4 import (
    build_model as build_v4,
    LightweightGNN, StableDetectorGraphConv, LayerScale
)

CUSTOM_OBJECTS = {
    "Custom>LightweightGNN": LightweightGNN,
    "Custom>StableDetectorGraphConv": StableDetectorGraphConv,
    "Custom>LayerScale": LayerScale,
    "LightweightGNN": LightweightGNN,
    "StableDetectorGraphConv": StableDetectorGraphConv,
    "LayerScale": LayerScale,
    "custom_loss": custom_loss,
}

def plot_residuals(det_ids, res_plus, res_minus, model_path, stage_label):
    mean_p  = np.nanmean(np.abs(res_plus), axis=0)
    std_p   = np.nanstd(np.abs(res_plus), axis=0)
    mean_m  = np.nanmean(np.abs(res_minus), axis=0)
    std_m   = np.nanstd(np.abs(res_minus), axis=0)

    plt.figure(figsize=(10, 5))
    plt.errorbar(det_ids, mean_p, yerr=std_p, marker='o', label='μ+ mean±σ')
    plt.errorbar(det_ids, mean_m, yerr=std_m, marker='s', label='μ- mean±σ')
    plt.axhline(0, linestyle='--', linewidth=1)
    plt.xlabel('Detector Layer (skipping masked slots)')
    plt.ylabel('Absolute Residual (predicted − true)')
    plt.title(f'Per-layer Absolute Residual ({stage_label.capitalize()})')
    plt.legend()
    plt.tight_layout()
    base     = os.path.splitext(os.path.basename(model_path))[0]
    fname    = f"{base}_{stage_label}_residuals.png"
    plot_dir = os.path.join(os.path.dirname(__file__), "plots")
    os.makedirs(plot_dir, exist_ok=True)
    plt.savefig(os.path.join(plot_dir, fname), dpi=140)
    plt.show()

def load_trained_model(X_sample, model_path, use_bn=False):
    D = int(X_sample.shape[1])
    E = int(X_sample.shape[2])

    if model_path.endswith(".weights.h5"):
        try:
            model = build_v3(
                num_detectors=D, num_elementIDs=E,
                base_filters=48, use_bn=use_bn,
                gnn_channels=24, gnn_layers=1
            )
            model.load_weights(model_path)
            print("[load] loaded weights into UGNN v3 architecture")
            return model
        except Exception as e_v3:
            print(f"[warn] UGNN v3 load_weights failed: {e_v3}")
        try:
            model = TrackFinder_unetpp.build_model(use_bn=use_bn, deep_supervision=False)
            model.load_weights(model_path)
            print("[load] loaded weights into UNet++ architecture (fallback)")
            return model
        except Exception as e_unet:
            raise RuntimeError(
                f"Failed to load weights into both UGNN v3 and UNet++ architectures: "
                f"{e_v3} | {e_unet}"
            )

    try:
        model = tf.keras.models.load_model(
            model_path, custom_objects=CUSTOM_OBJECTS, compile=False
        )
        print("[load] loaded full model with custom_objects (compile=False)")
        return model
    except Exception as e_full:
        print(f"[warn] full-model load failed: {e_full}")
        print("[fallback] Build UGNN v3 and load weights by_name(skip_mismatch=True)")
        try:
            model = build_v3(
                num_detectors=D, num_elementIDs=E,
                base_filters=48, use_bn=use_bn,
                gnn_channels=24, gnn_layers=1
            )
            model.load_weights(model_path, by_name=True, skip_mismatch=True)
            print("[load] by_name weights loaded into UGNN v3 (skip_mismatch=True)")
            return model
        except Exception as e_byname_v3:
            print(f"[warn] UGNN v3 by_name load failed: {e_byname_v3}")
        try:
            model = TrackFinder_unetpp.build_model(use_bn=use_bn, deep_supervision=False)
            model.load_weights(model_path, by_name=True, skip_mismatch=True)
            print("[load] by_name weights loaded into UNet++ (skip_mismatch=True)")
            return model
        except Exception as e_byname_unet:
            raise RuntimeError(
                f"Failed to recover from full-model load. "
                f"UGNN v3 by_name error: {e_byname_v3} | "
                f"UNet++ by_name error: {e_byname_unet}"
            )

def evaluate_model(root_file, model_path, use_bn=False):
    X_test, y_muPlus_test, y_muMinus_test = data_loader.load_data(root_file)
    if X_test is None:
        print("[error] failed to load test data.")
        return

    y_test = np.stack([y_muPlus_test, y_muMinus_test], axis=1)
    det_test, elem_test, _, _, _ = QTracker_prod.load_detector_element_data(root_file)

    mask = np.ones(62, dtype=bool)
    mask[6:12]   = False
    mask[54:62]  = False

    model = load_trained_model(X_test, model_path, use_bn=use_bn)

    y_pred = model.predict(X_test, verbose=1)  # (N, 2, D, E)
    softmax_mup = y_pred[:, 0, :, :]
    softmax_mum = y_pred[:, 1, :, :]

    y_p_raw = tf.argmax(softmax_mup, axis=-1, output_type=tf.int32).numpy()
    y_m_raw = tf.argmax(softmax_mum, axis=-1, output_type=tf.int32).numpy()

    y_p_true = y_test[:, 0, :].astype(np.int32)
    y_m_true = y_test[:, 1, :].astype(np.int32)

    raw_p_res = y_p_raw - y_p_true
    raw_m_res = y_m_raw - y_m_true

    print("\n--- Raw Residuals (Before Refinement, all events) ---")
    print("Det |  μ+ mean  |  μ+ std   |  μ- mean  |  μ- std")
    for det in np.where(mask)[0]:
        m_p, s_p = np.mean(np.abs(raw_p_res[:, det])), np.std(np.abs(raw_p_res[:, det]))
        m_m, s_m = np.mean(np.abs(raw_m_res[:, det])), np.std(np.abs(raw_m_res[:, det]))
        print(f"{det+1:3d} | {m_p:8.3f} | {s_p:8.3f} | {m_m:8.3f} | {s_m:8.3f}")

    big_raw, big_true = [], []
    for i, res in enumerate(np.abs(raw_p_res[:, 0])):
        if res > 0:
            big_raw.append(y_p_raw[i, 0])
            big_true.append(y_p_true[i, 0])
    df = pd.DataFrame({'Raw μ+ Values': big_raw, 'True μ+ Values': big_true})
    print(f'\n{df.head(20)}')

    dets_used = (np.where(mask)[0] + 1)
    plot_residuals(dets_used, raw_p_res[:, mask], raw_m_res[:, mask], model_path, 'raw')

    acc_p_exact = np.mean(np.abs(raw_p_res) == 0)
    acc_m_exact = np.mean(np.abs(raw_m_res) == 0)
    print(f"\nRaw μ+ accuracy: {acc_p_exact:.4f}")
    print(f"Raw μ- accuracy: {acc_m_exact:.4f}")

    acc_p_d2 = np.mean(np.abs(raw_p_res) <= 2)
    acc_m_d2 = np.mean(np.abs(raw_m_res) <= 2)
    print(f"\nRaw μ+ distance-based accuracy: {acc_p_d2:.4f}")
    print(f"Raw μ- distance-based accuracy: {acc_m_d2:.4f}")

    ref_p, ref_m = refine_hit_arrays(y_p_raw, y_m_raw, det_test, elem_test)
    ref_p_res = ref_p - y_p_true
    ref_m_res = ref_m - y_m_true

    print("\n--- Refined Residuals (After Refinement, all events) ---")
    print("Det |  μ+ mean  |  μ+ std   |  μ- mean  |  μ- std")
    for det in np.where(mask)[0]:
        m_p, s_p = np.mean(np.abs(ref_p_res[:, det])), np.std(np.abs(ref_p_res[:, det]))
        m_m, s_m = np.mean(np.abs(ref_m_res[:, det])), np.std(np.abs(ref_m_res[:, det]))
        print(f"{det+1:3d} | {m_p:8.3f} | {s_p:8.3f} | {m_m:8.3f} | {s_m:8.3f}")

    plot_residuals(dets_used, ref_p_res[:, mask], ref_m_res[:, mask], model_path, 'refined')

    acc_p_exact = np.mean(np.abs(ref_p_res) == 0)
    acc_m_exact = np.mean(np.abs(ref_m_res) == 0)
    print(f"\nRefined μ+ accuracy: {acc_p_exact:.4f}")
    print(f"Refined μ- accuracy: {acc_m_exact:.4f}")

    acc_p_d2 = np.mean(np.abs(ref_p_res) <= 2)
    acc_m_d2 = np.mean(np.abs(ref_m_res) <= 2)
    print(f"\nRefined μ+ distance-based accuracy: {acc_p_d2:.4f}")
    print(f"Refined μ- distance-based accuracy: {acc_m_d2:.4f}")

    print("\n--- Raw Absolute Residuals (Before Refinement) ---")
    print("μ+ mean  |  μ+ std   |  μ- mean  |  μ- std")
    m_p, s_p = np.mean(np.abs(raw_p_res)),  np.std(np.abs(raw_p_res))
    m_m, s_m = np.mean(np.abs(raw_m_res)),  np.std(np.abs(raw_m_res))
    print(f"{m_p:8.3f} | {s_p:8.3f} | {m_m:8.3f} | {s_m:8.3f}")

    print("\n--- Refined Absolute Residuals (After Refinement) ---")
    print("μ+ mean  |  μ+ std   |  μ- mean  |  μ- std")
    m_p, s_p = np.mean(np.abs(ref_p_res)),  np.std(np.abs(ref_p_res))
    m_m, s_m = np.mean(np.abs(ref_m_res)),  np.std(np.abs(ref_m_res))
    print(f"{m_p:8.3f} | {s_p:8.3f} | {m_m:8.3f} | {s_m:8.3f}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Evaluate pre-trained TrackFinder models.")
    parser.add_argument("root_file",  type=str, help="Path to the val/test ROOT file.")
    parser.add_argument("model_path", type=str, help="Path to the saved model file (.h5 / .keras / .weights.h5).")
    parser.add_argument("--batch_norm", type=int, default=0, help="Use BN (0/1).")
    args = parser.parse_args()

    print(f"\nResults for {args.model_path}...")
    evaluate_model(args.root_file, args.model_path, bool(args.batch_norm))
